...
// index.html mein add karo: <script src="books-modal.js"></script>
// ============================================================

const NMC_TAG      = "nmc007-21";       // Apna Amazon tracking ID
const GEMINI_KEY   = "AIzaSyCQMog_2AHmHs8imdGGXFU80495XebmdPI";  // Apni Gemini API key yahan
...
(function buildOverlay() {
  ...
  div.innerHTML = `
    ...
    <div class="nmc-ovl-body">
      ...
      <div class="nmc-view-more2"><a id="nmc-ovl-more" href="#" target="_blank" rel="noopener">View more on Amazon →</a></div>
    </div>
  ...
  toast.className = "nmc-toast"; toast.id = "nmc-toast";
  toast.innerHTML = `<span>✅</span><span id="nmc-toast-msg">PDF Downloaded!</span>`;
  document.body.appendChild(toast);
})();

// ── Inject strip into existing result modal ───────────────────
  ...
  if (modal) {
    const observer = new MutationObserver(() => {
      if (modal.style.display !== "none" && modal.style.display !== "") {
        const examEl = document.getElementById("examName") || document.getElementById("exam-name") || document.getElementById("examname");
        const exam = examEl ? examEl.value.trim() : "";
        nmcFetchStrip(exam || "competitive exam");
      }
    });
...
async function nmcFetchBooks(examName) {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({
      contents: [{
        parts: [{
          text: `You are a book recommender for Indian competitive exam students.
Exam: "${examName}"
Return ONLY a valid JSON array of 3 books. No markdown, no explanation, no extra text.
[{"title":"Book Title - Author Name","search":"short amazon search query"},{"title":"Book Title - Author Name","search":"short amazon search query"},{"title":"Book Title - Author Name","search":"short amazon search query"}]
Rules: real books on Amazon India, standard books for this specific exam, search = short precise query.`
        }]
      }],
...
function nmcURL(q) { return `https://www.amazon.in/s?k=${encodeURIComponent(q)}&tag=${NMC_TAG}`; }
...
// ── Overlay (full screen after PDF download) ──────────────────
window.nmcShowOverlay = function (examName) {
  const ovl = document.getElementById("nmc-overlay");
...
// ── Hook into existing downloadResult ─────────────────────────
// main.js ka downloadResult run hone ke baad yeh call karo:
// Existing downloadResult ke end mein add karo:
